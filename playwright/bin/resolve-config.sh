...
# CMS credentials are NOT copied here. The playwright container reads them directly
# from the project's own env files via docker-compose `env_file` (see
# docker-compose.playwright.yaml), so SHOPWARE_ACCESS_KEY_ID / SHOPWARE_SECRET_ACCESS_KEY /
# WP_USERNAME / WP_PASSWORD stay in ONE place: .env / .env.local / .env.test. We only
# export the DIRECTORY that holds those files (absolute host path, robust whether the
# app lives in the project root or a subfolder), plus a base-URL fallback because the
# project .env usually does not set APP_URL / WP_BASE_URL (they default to the DDEV URL).
CMS_ENV_DIR="${PROJECT_ENV_DIR:-${PROJECT_ROOT}}"
CMS_BASE_URL_FALLBACK="${DDEV_PRIMARY_URL:-http://web}"
...
# It carries ONLY the values docker-compose needs at interpolation time (image tag,
# CMS flags, the env-file directory and the base-URL fallback). No credentials — those
# come from the project env files via `env_file`. Safe to be git-tracked, but the add-on
# also gitignores it (post-start hook) since older versions wrote credentials here.
{
  printf '# GENERATED by the ddev-playwright add-on — DO NOT EDIT.\n'
  printf '# Regenerated on every ddev run. Credentials live in your project\n'
  printf '# .env / .env.local / .env.test, read directly by the container (env_file).\n'
  ...
  printf 'PLAYWRIGHT_CMS_ENV_DIR=%s\n' "${CMS_ENV_DIR}"
  printf 'PLAYWRIGHT_BASE_URL_FALLBACK=%s\n' "${CMS_BASE_URL_FALLBACK}"
} > "${ENV_FILE}"

# Base-URL fallback file, loaded as the FIRST env_file so a real APP_URL / WP_BASE_URL
# in the project .env(.local/.test) overrides it. Written into the app's env dir next
# to the files it complements; git-ignored via the post-start hook. Only base URLs go
# here — never credentials (those come straight from the project env files).
FALLBACK_ENV="${CMS_ENV_DIR}/.ddev-playwright-fallback.env"
{
  printf '# GENERATED by the ddev-playwright add-on — DO NOT EDIT (git-ignored).\n'
  printf '# Base-URL defaults only; overridden by APP_URL / WP_BASE_URL in your .env(.local).\n'
  if [ "${IS_SHOPWARE}" = "true" ]; then
    printf 'APP_URL=%s\n' "${CMS_BASE_URL_FALLBACK}"
  fi
  if [ "${IS_WORDPRESS}" = "true" ]; then
    printf 'WP_BASE_URL=%s\n' "${CMS_BASE_URL_FALLBACK}"
    printf 'WP_USERNAME=admin\n'
    printf 'WP_PASSWORD=password\n'
  fi
} > "${FALLBACK_ENV}" 2>/dev/null || true
